...
MongoDB.prototype.execute = function(model, command) {
  ...
  // mice workaround for cache
  var miceOpts = null;
  var isMiceOptsExists = args[0] && args[0].isMiceOpts;
  if (isMiceOptsExists) {
    miceOpts = args[0];
    args = args.slice(1);
  };
  ...
  var context = {
    ...
    collection: collection, req: {
      command: command,
      params: args,
      miceOpts: miceOpts, // mice workaround for cache
    },
  };
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  // mice workaround for cache
  var savedFilter = _.cloneDeep(filter);
  var miceOpts = {
    isMiceOpts: true,
    filter: savedFilter,
  };
  ...
  if (fields) {
    // this.execute(model, 'find', query, fields, processResponse); // original
    this.execute(model, 'find', miceOpts, query, fields, processResponse); // mice workaround for cache
  } else {
    // this.execute(model, 'find', query, processResponse); // original
    this.execute(model, 'find', miceOpts, query, processResponse); // mice workaround for cache
  }
  ...
  function processResponse(err, cursor) {
    ...
    var endResponse = function(result) {
    ...
    // mice workaround for cache
    var isCachedResponse = !cursor.cursorState;
    debug('isCachedResponse %s', isCachedResponse);
    if (isCachedResponse) {
      endResponse(cursor);
      return;
    }
    ...
    cursor.toArray(function(err, data) {
      ...
      // mice workaround for cache
      self.dataSource.emit('mice:dataForCache', {
        data: objs,
        filter: savedFilter,
        modelName: model,
      });
